...
void CreateVirtualStack(FctPtr function, INT16U NUMBER_OF_STACKED_BYTES)
{  
    ...
    *stk++  = (INT8U)((INT16U)(function)) & 0x00FF;     // TOSL
    *stk++  = (INT8U)((INT16U)(function) >> 8);         // TOSH
    *stk++  = (INT8U)0;                                // TOSU
...
void HighInterrupt( void )
{
    ...
    if( PIR1bits.CCP1IF )
    {        
        __asm goto _TickTimerHandler  __endasm;
    }
    
    /* Was the interrupt a byte being received? */
    if( PIR1bits.RCIF )
    {
        //__asm goto _SerialRx        __endasm;
    }

    /* Was the interrupt a serial transmission completed? */
    if( PIR1bits.TXIF )
    {
        //__asm goto _SerialTx __endasm;
    }
}
#pragma code
//----------------------------------------------------------------------------
